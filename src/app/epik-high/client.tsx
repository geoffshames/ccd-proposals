...
const soldPercent = (sold: number, sellable: number) => `${(sold / sellable * 100).toFixed(1)}%`;
const sourceLinks = [
...
export default function EpikHighClient({ copy, locale = "en" }: {copy: Copy; locale?: "en" | "ko"}) {
  ...
  const mail = (budget: 10000 | 15000) => {
    ...
    const creatorNote = locale === "ko"
      ? `\n총 ${money(budget)} 예산에는 크리에이터 섭외·협상 운영비 $1,000과 실집행 광고비의 15%인 캠페인 운영비가 포함됩니다.\n`
      : `\nThe all-in ${money(budget)} budget includes $1,000 for creator outreach and negotiation management, plus campaign management at 15% of media spend.\n`;
    return `mailto:geoff@crowdcontroldigital.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body + creatorNote)}`;
  };
  ...
  return <main className={styles.page} lang={locale} id="top">
    ...
    <section className={styles.section} id="markets">
      ...
      <div className={styles.ticketSnapshot}><h3>{t('ticketSnapshotTitle')}</h3><dl><div><dt>{t('ticketsSold')}</dt><dd>{count(tourData.ticketReport.sold)} <small>/ {count(tourData.ticketReport.sellable)}</small></dd></div><div><dt>{t('sellThrough')}</dt><dd>{soldPercent(tourData.ticketReport.sold,tourData.ticketReport.sellable)}</dd></div><div><dt>{t('ticketsOpen')}</dt><dd>{count(tourData.ticketReport.open)}</dd></div></dl><p className={styles.note}>{t('ticketSnapshotNote')}</p></div>
      ...
      <div className={styles.tourTable}>
        ...
        {tourData.tour.map((show)=>{const support=tier===10?show.support10:show.support15;const amount=tier===10?show.budget10:show.budget15;return <article className={styles.showRow} key={show.date+show.city}>
          <div className={styles.showIdentity}><time dateTime={show.date}>{day(show.date)}</time><div><h4>{show.city}</h4><span>{show.venue}</span><div className={styles.ticketCounts}><span>{t('ticketsSold')}: {count(show.ticketCount.sold)} / {count(show.ticketCount.sellable)} · {soldPercent(show.ticketCount.sold,show.ticketCount.sellable)}</span><b>{count(show.ticketCount.open)} {t('ticketsOpen')}</b></div><Out href={show.url}>{t('tickets')}</Out></div></div>
      ...
      <p className={styles.note}>{t('soldoutNote')}</p><p className={styles.rule}>{t('priorityRule')}</p>
    </section>
    <section className={styles.section} id="creative">
      ...
      <div className={styles.creatorSection} id="creators"><span className={styles.eyebrow}>13 {locale==='en'?'markets':'개 도시'} · {locale==='en'?'Ticket exchange':'티켓 교환'}</span><h3>{t('creatorTitle')}</h3><p className={styles.creatorIntro}>{t('creatorIntro')}</p><p className={styles.rule}><a href="#creator-management">{t('creatorFeeTitle')} · $1,000</a></p>
    ...
    <section className={styles.section} id="investment">
      ...
      <div className={styles.packages}>{([10,15] as const).map((n)=>{const higher=n===15;const name=higher?'expanded':'focused';const budget=tourData.investment.packages[n];const rows=[{key:'media',amount:budget.media},{key:'adaptation',amount:0},{key:'management',amount:budget.management},{key:'creatorFeeTitle',amount:budget.creator},{key:'vendors',amount:0}];return <article key={n} className={`${styles.package} ${higher?styles.packageRecommended:''}`}>
        ...
        <dl className={styles.costs}>{rows.map(row=><div key={row.key}><dt>{t(row.key as Key)}</dt><dd>{money(row.amount)}</dd></div>)}</dl><div className={styles.total}><b>{t('total')}</b><b>{money(n*1000)}</b></div><p className={styles.deliverables}>{t((name+'Deliverables') as Key)}</p><a className={higher?styles.button:styles.outlineButton} href={mail(higher?15000:10000)}>{t(higher?'choose15':'choose10')} ↗</a>
      </article>})}</div>
      <p className={styles.note}>{t('masterNote')}</p>
      <aside className={styles.creatorFee} id="creator-management"><div><h3>{t('creatorFeeTitle')}</h3><strong>$1,000 <small>USD</small></strong></div><p>{t('creatorFeeBody')}</p><p className={styles.feeTotal}>{t('creatorFeeTotal')}</p></aside>
      <div className={styles.delta}><h3>{t('deltaTitle')}</h3>{['Media','Creative','Mgmt'].map(key=><div key={key}><b>{t(('delta'+key) as Key)}</b><p>{t(('delta'+key+'Body') as Key)}</p></div>)}</div>
      <h3 className={styles.allocationTitle}>{t('allocationTitle')}</h3><div className={styles.allocationWrap}><table className={styles.allocation}><thead><tr><th>{t('line')}</th><th>$10,000</th><th>$15,000</th></tr></thead><tbody>{[{key:'ticketCore',a:5100,b:7400},{key:'ticketLight',a:0,b:500},{key:'ongoing',a:tourData.investment.packages[10].release,b:tourData.investment.packages[15].release},{key:'post',a:1300,b:2600},{key:'mediaTotal',a:tourData.investment.packages[10].media,b:tourData.investment.packages[15].media}].map(row=><tr key={row.key}><th>{t(row.key as Key)}</th><td>{money(row.a)}</td><td>{money(row.b)}</td></tr>)}</tbody></table></div><p className={styles.note}>{t('mediaNote')}</p><p className={styles.note}>{t('poolNote')}</p><aside className={styles.promoterPivot}><h3>{t('promoterPivotTitle')}</h3><p>{t('promoterPivotBody')}</p><p>{t('promoterPivotTracking')}</p><p className={styles.note}>{t('promoterPivotBudget')}</p></aside><p className={styles.assumption}>{t('calendarAssumption')}</p><details className={styles.details}><summary>{locale==='en'?'Scope boundaries + team inputs':'업무 범위 및 팀 제공 사항'}</summary><p>{t('exclusions')}</p></details>
    </section>
