...
export default function EpikHighClient({ copy, locale = "en" }: {copy: Copy; locale?: "en" | "ko"}) {
  ...
  return <main className={styles.page} lang={locale} id="top">
    ...
    <section className={styles.section} id="investment">
      ...
      <div className={styles.packages}>{[10,15].map((n)=>{const higher=n===15;const name=higher?'expanded':'focused';const rows=[{key:'media',amount:n*1000*0.85},{key:'adaptation',amount:0},{key:'management',amount:n*1000*0.15},{key:'vendors',amount:0}];return <article key={n} className={`${styles.package} ${higher?styles.packageRecommended:''}`}>
      ...
      <h3 className={styles.allocationTitle}>{t('allocationTitle')}</h3><div className={styles.allocationWrap}><table className={styles.allocation}><thead><tr><th>{t('line')}</th><th>$10,000</th><th>$15,000</th></tr></thead><tbody>{[{key:'ticketCore',a:5100,b:5100},{key:'ticketExtra',a:0,b:2200},{key:'ticketLight',a:0,b:600},{key:'ongoing',a:2100,b:2250},{key:'post',a:1300,b:2600},{key:'mediaTotal',a:8500,b:12750}].map(row=><tr key={row.key}><th>{t(row.key as Key)}</th><td>{money(row.a)}</td><td>{money(row.b)}</td></tr>)}</tbody></table></div><p className={styles.note}>{t('mediaNote')}</p><p className={styles.note}>{t('poolNote')}</p><p className={styles.assumption}>{t('calendarAssumption')}</p><details className={styles.details}><summary>{locale==='en'?'Scope boundaries + team inputs':'업무 범위 및 팀 제공 사항'}</summary><p>{t('exclusions')}</p></details>
    </section>
