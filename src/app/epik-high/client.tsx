...
export default function EpikHighClient({ copy, locale = "en" }: {copy: Copy; locale?: "en" | "ko"}) {
  ...
  return <main className={styles.page} lang={locale} id="top">
    ...
    <section className={styles.section} id="music">
      ...
      <p className={styles.assumption}>{t('releaseAssumption')}</p>
    ...
    <section className={styles.section} id="waves">
      ...
      <figure className={styles.wave}><svg viewBox="0 0 1000 185" role="img" aria-label={t('waveCaption')}><path d="M20 155 H980" stroke="#555" strokeWidth="1"/><path d="M20 145 C160 145 245 35 430 35 C490 35 490 125 545 137" stroke="#FD3737" strokeWidth="5" fill="none"/><path d="M440 145 C485 145 480 10 500 10 C520 10 515 145 560 145" stroke="#FAFAFA" strokeWidth="5" fill="none"/><path d="M525 145 C570 140 620 52 700 52 C790 52 850 142 980 145" stroke="#FD3737" strokeWidth="4" strokeDasharray="10 7" fill="none"/><line x1="500" y1="0" x2="500" y2="172" stroke="#999" strokeDasharray="2 6"/><circle cx="500" cy="10" r="7" fill="#FAFAFA"/></svg><figcaption>{t('waveCaption')}</figcaption></figure>
    ...
    <section className={styles.section} id="markets">
      ...
      <div className={styles.tourTable}>
        ...
        {tourData.tour.map((show)=>{const support=tier===10?show.support10:show.support15;const amount=tier===10?show.budget10:show.budget15;return <article className={styles.showRow} key={show.date+show.city}>
          ...
          <div className={styles.showWindow}>{amount?<><span>{range(show.flightStart,show.flightEnd)}</span>{support==='full'&&<small>{t('recap')}: {range(show.recapStart,show.recapEnd)}</small>}</>:<span>{t('noFlight')}</span>}</div>
        </article>})}
      </div>
    ...
    <section className={styles.section} id="investment">
      ...
      <div className={styles.packages}>{[10,15].map((n)=>{const higher=n===15;const name=higher?'expanded':'focused';const rows=[{key:'media',amount:higher?11000:7000},{key:'adaptation',amount:higher?2250:1500},{key:'management',amount:higher?1750:1500},{key:'vendors',amount:0}];return <article key={n} className={`${styles.package} ${higher?styles.packageRecommended:''}`}>
      ...
      <h3 className={styles.allocationTitle}>{t('allocationTitle')}</h3><div className={styles.allocationWrap}><table className={styles.allocation}><thead><tr><th>{t('line')}</th><th>$10,000</th><th>$15,000</th></tr></thead><tbody>{[{key:'ticketCore',a:4800,b:4800},{key:'ticketExtra',a:0,b:2200},{key:'ticketLight',a:0,b:600},{key:'ongoing',a:1600,b:2400},{key:'post',a:600,b:1000},{key:'mediaTotal',a:7000,b:11000}].map(row=><tr key={row.key}><th>{t(row.key as Key)}</th><td>{money(row.a)}</td><td>{money(row.b)}</td></tr>)}</tbody></table></div><p className={styles.note}>{t('mediaNote')}</p><p className={styles.note}>{t('poolNote')}</p><p className={styles.assumption}>{t('calendarAssumption')}</p><details className={styles.details}><summary>{locale==='en'?'Scope boundaries + team inputs':'업무 범위 및 팀 제공 사항'}</summary><p>{t('exclusions')}</p></details>
    </section>
