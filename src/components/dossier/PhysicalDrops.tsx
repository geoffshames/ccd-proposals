...
export function PhysicalDrops() {
  const d = useDossier();
  const mockups = d.images?.productMockups ?? [];
          ...
          {d.physicalDrops.items.map((item) => (
            <article key={item.code} className="bg-bg p-7 sm:p-8">
              {(() => { const m = mockupFor(item.code); return m ? (
                <div className="relative aspect-[4/3] mb-6 -mx-7 sm:-mx-8 -mt-7 sm:-mt-8 overflow-hidden border-b border-white/[0.10] bg-black/40">
                  <img src={m.url} alt={m.caption ?? item.title} className="w-full h-full object-cover" />
                  {m.caption && (
